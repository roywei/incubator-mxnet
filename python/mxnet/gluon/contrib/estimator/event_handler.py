...
class CheckpointHandler(TrainBegin, BatchEnd, EpochEnd):
    ...
    def _save_symbol(self, estimator):
        ...
        # check if net is HybridBlock and already hybridized
        if hasattr(estimator.net, '_cached_graph') and not estimator.net._cached_graph:
            ...
            self.logger.info("Model architecture(symbol file) is not saved, please use HybridBlock "
                             "and HybridSequential to construct your model, then call net.hybridize() "
                             "before passing to Estimator in order to save model architecture as %s.",
                             symbol_file)
